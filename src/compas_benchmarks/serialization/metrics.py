...
* round-trip fidelity: exact equality of ``__data__`` and of the format-independent
  :func:`compas_benchmarks.hashing.canonical_hash` (this is where a lossy profile such as
  protobuf-float32 would show up).
"""
...
from compas_benchmarks.hashing import canonical_hash
...
def _hash_of(obj):
    ...
    if isinstance(obj, (list, tuple)):
        return [canonical_hash(item) for item in obj]
